...
export const EVENT_TYPES: { type: string; icon: string; keywords: string[]; gift: boolean }[] = [
...
}, penalizeNight: boolean): LuckyHour {
  const zhi = h.ganzhi.charAt(1);
  let score = 0;
  if (user.favorable.includes(h.element)) score += 2;
  else if (h.element === user.dayMaster) score += 1;
  else if (user.unfavorable.includes(h.element)) score -= 1;
  if (CHONG[zhi] === user.shengXiaoZhi || CHONG[user.shengXiaoZhi] === zhi) score -= 3;
  const idx = '子丑寅卯辰巳午未申酉戌亥'.indexOf(zhi);
  const hourIndex = idx < 0 ? 0 : idx;
  // 深夜时段（子/丑/寅：23:00-05:00）对多数事项不合适，做办的事减分
  if (penalizeNight && (hourIndex === 0 || hourIndex === 1 || hourIndex === 2)) score -= 1.5;
...
export function generateEventAdvice(input: { birth: BirthInput; eventType: string; date: string }): EventAdviceResult {
  ...
  const verdict: EventAdviceResult['verdict'] = score >= 80 ? '宜' : score >= 65 ? '平' : score >= 45 ? '慎' : '忌';

  // 4) 吉时建议（12 时辰按喜忌+冲生肖打分）
  const nightOk = et.type === '出行' || et.type === '安葬'; // 夜行/下葬类不限制深夜
  const hours = getHourPillars(input.date)
  ...
  if (verdict === '忌' || verdict === '慎') {
    if (nextGood) resolve.push(`建议改期：近期更宜「${et.keywords[0]}」的吉日是 ${nextGood}（${getAlmanac(nextGood).yi.filter(y => et.keywords.some(k => y.includes(k))).slice(0, 2).join('、')}）。`);
    else resolve.push('建议改期：再观察一段时日，另择黄历宜日进行。');
    if (luckyHours.length) resolve.push(`若日期无法更改，务必安排在吉时（如 ${luckyHours.map(h => h.label).join('、')}）进行。`);
    if (worst && !worst.good) resolve.push(`尽量避开 ${worst.label}时（${worst.ganzhi}）——${worst.score <= -2 ? '与您相冲或犯忌神' : '气场不旺'}。`);
  } else {
    if (luckyHours.length) resolve.push(`选吉时进行：${luckyHours.map(h => `${h.label}(${h.range})`).join('、')}。`);
  }
  if (bazi.favorable.length) resolve.push(`当日可穿/佩戴「${bazi.favorable.join('、')}」属相的衣物配饰助运。`);
...
export function findNextGoodDate(eventType: string, from: Date, days = 30): string | null {
  const et = EVENT_TYPES.find(e => e.type === eventType);
  if (!et) return null;
  for (let i = 1; i <= days; i++) {
    const d = new Date(from.getTime() + i * 86400000);
    const ds = toDateStr(d);
    const a = getAlmanac(ds);
    if (a.yi.some(y => et.keywords.some(k => y.includes(k)))) return ds;
  }
  return null;
}
