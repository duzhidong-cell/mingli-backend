...
if (existsSync(crawleeStorage)) {
  try {
    ...
    console.warn('[抓取] 清理 storage/ 失败:', (e as Error).message);
  }
}
...
const KEYWORDS = sources.keywords as string[];

function hasKeyword(text: string): string | null {
  const t = text || '';
...
function extractArticle($: CheerioCrawlingContext['$']): { title: string; content: string; publishedAt: string } {
  ...
  for (const sel of containers) {
    ...
    el.find('p').each((_, p) => {
      const t = $(p).text().trim();
      if (t.length > 5) paras.push(t);
    });
    if (paras.length > best.length) best = paras;
  }
  // 兜底：全文 p
  if (!best.length) {
    $('p').each((_, p) => {
      const t = $(p).text().trim();
      if (t.length > 5) best.push(t);
    });
  }
...
async function runSource(source: (typeof sources.sources)[number]): Promise<{ added: number; failed: boolean }> {
  let added = 0;
    ...
    async requestHandler(ctx: CheerioCrawlingContext) {
      ...
      if ((linkHit || depth >= 1) && pageTitle.length > 6) {
        ...
        if (content.length > 80) {
          ...
          const art: Omit<Article, 'id' | 'scrapedAt'> = {
  ...
  try {
    ...
    console.error(`[抓取] ${source.name} 失败:`, (e as Error).message);
...
export async function runScrapeOnce(): Promise<{ added: number; total: number }> {
  if (running) return { added: 0, total: getArticleCount() }; // 已有抓取在跑，直接跳过避免并发
  running = true;
  try {
    const now = Date.now();
    const results = await Promise.allSettled(
      (sources.sources as typeof sources.sources)
        .filter(s => s.enabled)
        .map(async (source) => {
    ...
    for (const res of results) {
      if (res.status === 'fulfilled') added += res.value.added;
    }
