import { computeBaZi, getAlmanac, getHourPillars } from './baziService';
import { ancientLib } from './ancientService';
import type { BirthInput, DailyLuckyResult, LuckyCard, Region } from '../types';
...
/** 五行 → 意象 */
const ELEMENT_CARDS: Record<string, { keyword: string; color: string; direction: string; desc: string }> = {
...
/** 天干 → 意象 */
const GAN_CARDS: Record<string, { keyword: string; desc: string }> = {
...
/** 地支 → 意象 */
const ZHI_CARDS: Record<string, { keyword: string; desc: string }> = {
...
function dayMasterSummary(b: { dayMaster: string; strength: string; favorable: string[]; unfavorable: string[] }): string {
  return `日主属「${b.dayMaster}」，身${b.strength}，喜用五行 ${b.favorable.join('、')}，忌 ${b.unfavorable.join('、')}`;
}

/** 时辰打分（与事项择吉一致）：喜用五行 + 日主 + 忌神 + 冲生肖 */
function scoreHour(
  h: { ganzhi: string; element: string; range: string; hourIndex: number },
  fav: string[], unf: string[], dayMaster: string, chongZhi: string | null,
): number {
  let s = 0;
  if (fav.includes(h.element)) s += 2;
  else if (h.element === dayMaster) s += 1;
  else if (unf.includes(h.element)) s -= 1;
  if (chongZhi && (h.ganzhi.charAt(1) === chongZhi)) s -= 3;
  return s;
}
 ...
 */
export function generateDailyLucky(input: { birth: BirthInput; date: string; region?: Region }): DailyLuckyResult {
  ...
  const hours = getHourPillars(input.date).map(h => ({
    ...h,
  ...
  const ancientSources: string[] = [];
  for (const kw of sourceKeys) {
    for (const h of ancientLib.search(kw, 2)) {
      if (!found.has(h.bookId)) {
        found.add(h.bookId);
        ancientSources.push(h.title);
        if (ancientSources.length >= 4) break;
      }
    }
    if (ancientSources.length >= 4) break;
  }
